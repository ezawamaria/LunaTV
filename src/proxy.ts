...
import { getAuthInfoFromCookie, validateAuthInfo } from '@/lib/auth';
...
async function handleAuthentication(
  ...
  if (
    ...
    if (isIPTrusted(clientIP, trustedNetworkConfig.trustedIPs)) {
      ...
      if (trustedNetworkConfig.blockAdminAccess && isAdminPath) {
        ...
        // 检查是否已经有有效的认证 cookie
        const existingAuth = await validateAuthInfo(
          getAuthInfoFromCookie(request),
        );
        if (existingAuth) {
  ...
  if (storageType === 'localstorage') {
    const validAuth = await validateAuthInfo(authInfo, { storageType });
    if (!validAuth) {
      return handleAuthFailure(request, pathname);
    }
    return response || NextResponse.next();
